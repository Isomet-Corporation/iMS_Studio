...
namespace iMS_Studio.ViewModel.Behaviour
{
    ...
    {
        private List<DockPaneViewModel> _panels;

        public AvalonDockLayoutViewModel(List<DockPaneViewModel> panels)
        {
            _panels = panels;
        }
        ...
        {
            string layoutFileName = System.IO.Path.Combine(App.DirAppData, App.LayoutFileName);

            if (System.IO.File.Exists(layoutFileName) == false)
            {
                layoutFileName = System.IO.Path.Combine(App.DirAppData, App.DefaultFileName);
                if (System.IO.File.Exists(layoutFileName) == false)
                    return;
            }
            ...
            {
                // Only open documents that exist in the project tree
                if (args.Model.GetType() == typeof(AvalonDock.Layout.LayoutDocument))
                {
                    bool match = false;
                    foreach(ImageDockWindowViewModel doc in
                        (s as XmlLayoutSerializer).Manager.DocumentsSource)
                    {
                        if (doc.ContentId == args.Model.ContentId)
                        {
                            // Match
                            match = true;
                        }
        ...
        private void ReloadContentOnStartUp(LayoutSerializationCallbackEventArgs args)
        {
            ...
            //args.Content = _panels.Find(x => x.ContentId == sId);
            //switch (sId)
            //{
            //    case "ProjectExplorer": args.Content = _panels.Find(x => x.ContentId == sId); break;
            //    case "CalibrationView": args.Content = MainViewModel.This.CalibrationVM; break;
            //    case "SignalPath": args.Content = MainViewModel.This.SignalPathVM; break;
            //    case "PlayConfiguration": args.Content = MainViewModel.This.PlayConfigurationVM; break;
            //    case "CompensationView": args.Content = MainViewModel.This.CompensationVM; break;
            //    default: args.Cancel = true; break;
            //}
        ...
        private void SaveDockingManagerLayout(string xmlLayout)
        {
            ...
            string fileName = System.IO.Path.Combine(App.DirAppData, App.LayoutFileName);

            File.WriteAllText(fileName, xmlLayout, System.Text.Encoding.Unicode);
        }
